...
app.get('/create-sample-user', async (req, res) => {
    try {
        ...
        await user.save();
        res.json({ message: 'Sample user created', user });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Error creating sample user' });
    }
});

// Similarly, create sample data for other schemas if needed
